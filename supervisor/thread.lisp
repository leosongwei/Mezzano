...
(defun debug-print-all-threads ()
  ...
  (do ((thread *all-threads* (thread-global-next thread)))
    ((null thread))
    (debug-print-line "Thread: " (thread-name thread)
                      "is-driver: " (thread-is-driver thread)))
  (debug-print-line "==========================================================="))
  ...
  (do ((thread *all-threads* (thread-global-next thread)))
    ...
    (if (= 1 (thread-is-driver thread))
      (progn (debug-print-line "Terminate driver: " (thread-name thread))
             (setf (thread-state thread) :dead)
             (when (thread-global-next thread)
               (setf (thread-global-prev (thread-global-next thread))
                     (thread-global-prev thread)))
             (when (thread-global-prev thread)
               (setf (thread-global-next (thread-global-prev thread))
